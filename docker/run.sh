...
ELASTIC_USERNAME=''
ELASTIC_PASSWORD=''
docker run -p 18080:8080 -e "N_CPUS=${N_CPUS}" -e "LOG_LEVEL=${LOG_LEVEL}" -e "API_DB_ENDPOINT=${API_DB_ENDPOINT}" -e "SH_DB_ENDPOINT=${SH_DB_ENDPOINT}" -e "AUTH0_DOMAIN=${AUTH0_DOMAIN}" -e "AUTH0_CLIENT_ID=${AUTH0_CLIENT_ID}" -e "AUTH0_USERNAME_CLAIM=${AUTH0_USERNAME_CLAIM}" -e "ELASTIC_URL=${ELASTIC_URL}" -e "ELASTIC_USERNAME=${ELASTIC_USERNAME}" -e "ELASTIC_PASSWORD=${ELASTIC_PASSWORD}" -it "${DOCKER_USER}/dev-analytics-affiliation-api" "/usr/bin/main"
