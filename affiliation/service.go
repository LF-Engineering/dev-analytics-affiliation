...
func (s *service) GetTopContributorsCSV(ctx context.Context, params *affiliation.GetTopContributorsCSVParams) (f io.ReadCloser, err error) {
	...
	for index, contributor := range topContributors.Contributors {
		row := []string{
			...
			strconv.FormatInt(contributor.GithubPullRequestsPrsCreated, 10),
			strconv.FormatInt(contributor.GithubPullRequestsPrsMerged, 10),
			strconv.FormatInt(contributor.GithubPullRequestsPrsOpen, 10),
			strconv.FormatInt(contributor.GithubPullRequestsPrsClosed, 10),
