...
if [ ! -z "$DEBUG" ]
then
  echo curl -i -s -H "Origin: https://insights.test.platform.linuxfoundation.org" -H "Authorization: Bearer ${JWT_TOKEN}" -XGET "${API_URL}/v1/affiliation/${project}/list_organizations?q=${q}&rows=${rows}&page=${page}"
